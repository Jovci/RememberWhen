...
export default async function PrivateMapPage() {
  ...
  const headerMap = await HeaderMap({ className: '' }); // Await the result of HeaderMap

  return (
    <div className="min-h-screen flex flex-col">
      <div className="fixed z-10 w-full">{headerMap}</div> {/* Fixed header */}
      <FullScreenMap
        userId={user.id}
        initialMarkers={formattedMarkers || []}
        isPrivateMap={true}
      />
    </div>
  );
